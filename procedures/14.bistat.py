...
def BiStat(pref, epi_cat1, epi_cat2):
    f = open('stdout_files/stdout_from_Bistat_normuti.txt', 'w')
    cmdargs = split('3dLocalBistat -nbhd SPHERE(-1) -stat normuti -prefix BiStat_Res/%(pref)s %(epi_cat1)s %(epi_cat2)s' % locals())   # with spearman flag
    call(cmdargs, stdout = f, stderr = STDOUT)
    f.close()
...
def meanRes(pref, epi1, epi2):
    ...
    cmdargs = split('3dMean -prefix BiStat_Res/%(pref)s %(epi1)s %(epi2)s' % locals())
    call(cmdargs, stdout = f, stderr = STDOUT)
    f.close() 
...
def meanRes2(pref, epis):
    ...
    '''
    f = open('stdout_files/stdout_from_3dmean.txt', 'w')
    cmdargs = split('3dMean -prefix BiStat_Res/%(pref)s %(epis)s' % locals())
    call(cmdargs, stdout = f, stderr = STDOUT)
    f.close() 
...
if __name__ == "__main__":
    for ss in subj_list:
        ...
        tcorr_suf = '_bistat_normuti_out'   # Set a common output prefix. Easier to switch between correlation type in the function above.  
        for seg in segments:
            ...
            for m in ('V', 'A'):
                ...
                epi1 = 'BiStat_Res/%(seg)s_%(m)s.1_%(ss)s%(tcorr_suf)s+orig' % locals()
                epi2 = 'BiStat_Res/%(seg)s_%(m)s.2_%(ss)s%(tcorr_suf)s+orig' % locals()
                pref = '%(seg)s_%(m)s_%(ss)s%(tcorr_suf)s+orig' % locals()
                meanRes(pref, epi1, epi2)

        for m in ('AV', 'A', 'V', 'lowlev'):
            epi_list = ' '.join(['BiStat_Res/%(seg)s_%(m)s_%(ss)s%(tcorr_suf)s+orig' % locals() for seg in segments]) 
