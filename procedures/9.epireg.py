...
def applywarpFLIRT(ss, input, extrt1, out):
    f = open('stdout_files/stdout_from_applywarpFLIRT.txt', 'w')
    cmdargs = split('applywarp -i %(input)s -r %(extrt1)s -o %(out)s --premat=epi2anat_%(ss)s_sess1_meanepi_mprage1.mat' % locals())
    call(cmdargs, stdout = f, stderr = STDOUT)
    f.close()
...
if __name__ == "__main__":
    ...
    for ss in subj_list:
        ...
        #input = 'epi2anat_%(ss)s_%(brain)s.nii.gz' % locals()
        coeff = '%(ss)s.%(brain)s.gert_reco.anat/T1_to_MNI_nonlin_coeff.nii.gz' % locals()
        out = 'highres_fnirted_%(ss)s_sess1_meanepi_%(brain)s' % locals()
        ...
        '''
        for m in ('A', 'V', 'lowlev'):
            input = '%(m)s_%(ss)s_tcorr_out_spearman_mean.nii.gz' % locals()
            outFL = 'highres_flirted_%(ss)s_%(m)s' % locals()
            applywarpFLIRT(ss, input, extrt1, outFL)

            input = '%(outFL)s.nii.gz' % locals()
            outFN = 'highres_fnirted_%(ss)s_%(m)s' % locals()
